...
public:
  ...
  Event::DispatcherPtr allocateDispatcherHostOnly(Event::TimeSystem& time_system) override {
    return Event::DispatcherPtr{allocateDispatcherHostOnly_(time_system)};
  }

  MOCK_METHOD1(allocateDispatcher_, Event::Dispatcher*(Event::TimeSystem&));
  MOCK_METHOD1(allocateDispatcherHostOnly_, Event::Dispatcher*(Event::TimeSystem&));
