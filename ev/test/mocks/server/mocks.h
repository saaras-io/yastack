...
public:
  ...
  MOCK_METHOD0(ffProcId, uint32_t());
  MOCK_METHOD0(ffProcType, std::string());
  MOCK_METHOD0(ffConfigPath, std::string());
  MOCK_METHOD0(hostOnly, uint8_t());
...
public:
  ...
  WorkerPtr createWorker(Envoy::Event::Dispatcher& dispatcher) override { return WorkerPtr{createWorker_(dispatcher)}; }

  MOCK_METHOD0(createWorker_, Worker*());
  MOCK_METHOD1(createWorker_, Worker*(Envoy::Event::Dispatcher&));
};
