...
class ConfigImpl : public ConfigImpl {
public:
...
virtual_hosts:
  ...
  EXPECT_THROW_WITH_REGEX(ConfigImpl(parseRouteConfigurationFromV2Yaml(invalid_virtual_cluster),
...
virtual_hosts:
  - name: www2
    ...
    EXPECT_THROW_WITH_MESSAGE(ConfigImpl config(route_config, factory_context, true),
...
virtual_hosts:
  ...
  EXPECT_NO_THROW(ConfigImpl(parseRouteConfigurationFromV2Yaml(value_with_regex_chars),
                                 factory_context, true));
    ...
    std::string json = R"EOF(
{
  ...
  ConfigImpl& config() {
    if (config_ == nullptr) {
      config_ = std::unique_ptr<ConfigImpl>{
          new ConfigImpl(route_config_, factory_context_, true)};
    }
    return *config_;
  }
...
virtual_hosts:
  ...
  NiceMock<Server::Configuration::MockFactoryContext> factory_context;
  EXPECT_THROW_WITH_MESSAGE(
      ConfigImpl(parseRouteConfigurationFromV2Yaml(yaml), factory_context, true),
...
TEST(RoutePropertyTest, excludeVHRateLimits) {
  ...
  config_ptr.reset(
      new ConfigImpl(parseRouteConfigurationFromJson(json), factory_context, true));
...
virtual_hosts:
  ...
  NiceMock<Server::Configuration::MockFactoryContext> factory_context;
  ConfigImpl config(parseRouteConfigurationFromV2Yaml(RouteDynPathRedirect), factory_context,
                        true);
...
virtual_hosts:
  ...
  NiceMock<Server::Configuration::MockFactoryContext> factory_context;
  ConfigImpl config(parseRouteConfigurationFromV2Yaml(ExplicitIdleTimeot), factory_context,
                        true);
  Http::TestHeaderMapImpl headers = genRedirectHeaders("idle.lyft.com", "/regex", true, false);
  const RouteEntry* route_entry = config.route(headers, 0)->routeEntry();
...
virtual_hosts:
  ...
  EXPECT_THROW_WITH_MESSAGE(
      ConfigImpl(parseRouteConfigurationFromV2Yaml(yaml), factory_context_, true),
