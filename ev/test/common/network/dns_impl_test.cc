...
public:
  ...
  int64_t socket(Address::SocketType type) const override { return instance_.socket(type); }
  Address::Type type() const override { return instance_.type(); }

  Envoy::Network::Address::SocketProvider
      provider() const override { return instance_.provider();}
  void provider(Envoy::Network::Address::SocketProvider p) override {
      instance_.provider(p);
  }
  int64_t socket(uint8_t provider, Envoy::Network::Address::SocketType type) const override {
      return instance_.socket(provider, type);
  }
