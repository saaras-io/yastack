...
namespace Envoy {
FakeStream::FakeStream(FakeHttpConnection& parent, Http::StreamEncoder& encoder, Event::TestTimeSystem& time_system)
    : parent_(parent), encoder_(encoder), time_system_(time_system) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::FakeStream() register this with stream callbacks ",static_cast<void *>(this), std::this_thread::get_id());
  encoder.getStream().addCallbacks(*this);
}

void FakeStream::decodeHeaders(Http::HeaderMapPtr&& headers, bool end_stream) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::decodeHeaders()",static_cast<void *>(this), std::this_thread::get_id());
...
void FakeStream::decodeData(Buffer::Instance& data, bool end_stream) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::decodeData()",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
  body_.add(data);
  setEndStream(end_stream);
  decoder_event_.notifyOne();
...
void FakeStream::decodeTrailers(Http::HeaderMapPtr&& trailers) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::decodeTrailers()",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
  setEndStream(true);
...
void FakeStream::encode100ContinueHeaders(const Http::HeaderMapImpl& headers) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::encode100ContinueHeaders()",static_cast<void *>(this), std::this_thread::get_id());
...
void FakeStream::encodeHeaders(const Http::HeaderMapImpl& headers, bool end_stream) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::encodeHeaders()",static_cast<void *>(this), std::this_thread::get_id());
  std::shared_ptr<Http::HeaderMapImpl> headers_copy(
      new Http::HeaderMapImpl(static_cast<const Http::HeaderMap&>(headers)));
...
void FakeStream::encodeData(uint64_t size, bool end_stream) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::encodeData()",static_cast<void *>(this), std::this_thread::get_id());
...
void FakeStream::encodeTrailers(const Http::HeaderMapImpl& trailers) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::encodeTrailers()",static_cast<void *>(this), std::this_thread::get_id());
...
void FakeStream::encodeResetStream() {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::encodeResetStream()",static_cast<void *>(this), std::this_thread::get_id());
  parent_.connection().dispatcher().post(
...
void FakeStream::onResetStream(Http::StreamResetReason) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::onResetStream(), set saw_reset_, decoder_event_.notifyOne()",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
  saw_reset_ = true;
  decoder_event_.notifyOne();
}
...
AssertionResult FakeStream::waitForHeadersComplete(milliseconds timeout) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::waitForHeadersComplete(), while(!headers_) wait for decoder_event_",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
...
AssertionResult FakeStream::waitForData(Event::Dispatcher& client_dispatcher, uint64_t body_length,
  ...
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::waitForData(), expected body not recvd, run non-blocking poll on dispatcher passed as arg",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
...
AssertionResult FakeStream::waitForEndStream(Event::Dispatcher& client_dispatcher,
        milliseconds timeout) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::waitForEndStream(), while(!end_stream), run non-blocking on dispatcher passed as arg",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
  auto start_time = time_system_.monotonicTime();
  while (!end_stream_) {
    if (time_system_.monotonicTime() >= start_time + timeout) {
...
AssertionResult FakeStream::waitForReset(milliseconds timeout) {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::waitForReset(), wait onf decoder_event if !saw_reset_",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
  auto start_time = time_system_.monotonicTime();
  while (!saw_reset_) {
    if (time_system_.monotonicTime() >= start_time + timeout) {
...
void FakeStream::startGrpcStream() {
  ENVOY_LOG(critical, "this [{}] tid[{}] FakeStream::startGrpcStream(), invoke encodeHeaders",static_cast<void *>(this), std::this_thread::get_id());
...
FakeHttpConnection::FakeHttpConnection(SharedConnectionWrapper& shared_connection,
  ...
  ENVOY_LOG_MISC(critical, "this [{}] tid[{}] FakeHttpConnection::FakeHttpConnection(), create ServerConnectionImpl and addReadFilter",static_cast<void *>(this), std::this_thread::get_id());
  if (type == Type::HTTP1) {
...
AssertionResult FakeConnectionBase::close(std::chrono::milliseconds timeout) {
  ENVOY_LOG_MISC(critical, "this [{}] tid[{}] FakeHttpConnection::close(), invoke connection_.close() if !disconnected_ ",static_cast<void *>(this), std::this_thread::get_id());
...
AssertionResult FakeConnectionBase::readDisable(bool disable, std::chrono::milliseconds timeout) {
  ENVOY_LOG_MISC(critical, "this [{}] tid[{}] FakeHttpConnection::readDisable() ",static_cast<void *>(this), std::this_thread::get_id());
  return shared_connection_.executeOnDispatcher(
...
AssertionResult FakeConnectionBase::enableHalfClose(bool enable,
  ...
  ENVOY_LOG_MISC(critical, "this [{}] tid[{}] FakeHttpConnection::enableHalfClose() ",static_cast<void *>(this), std::this_thread::get_id());
  return shared_connection_.executeOnDispatcher(
...
Http::StreamDecoder& FakeHttpConnection::newStream(Http::StreamEncoder& encoder) {
  ENVOY_LOG_MISC(critical, "this [{}] tid[{}] FakeHttpConnection::newStream() enqueue on new_stream_",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
...
AssertionResult FakeConnectionBase::waitForDisconnect(bool ignore_spurious_events,
                                                      milliseconds timeout) {
  ENVOY_LOG_MISC(critical, "this [{}] tid[{}] FakeHttpConnection::waitForDisconnect() while(!dosconnected_)",static_cast<void *>(this), std::this_thread::get_id());
...
AssertionResult FakeConnectionBase::waitForHalfClose(bool ignore_spurious_events,
                                                     milliseconds timeout) {
  auto end_time = time_system_.monotonicTime() + timeout;
  ENVOY_LOG_MISC(critical, "this [{}] tid[{}] FakeHttpConnection::waitForHalfClose() while(!half_closed_)",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
  while (!half_closed_) {
    if (time_system_.monotonicTime() >= end_time) {
...
AssertionResult FakeHttpConnection::waitForNewStream(Event::Dispatcher& client_dispatcher,
                                                     ...
                                                     milliseconds timeout) {
  auto end_time = time_system_.monotonicTime() + timeout;
  ENVOY_LOG_MISC(critical, "this [{}] tid[{}] FakeHttpConnection::waitForNewStream() while(!new_streams.empty())",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
...
FakeUpstream::FakeUpstream(const std::string& uds_path, FakeHttpConnection::Type type,
  ...
  ENVOY_LOG(critical, "this [{}] tid [{}] starting fake server on unix domain socket {}",static_cast<void *>(this), std::this_thread::get_id(), uds_path);
}
...
FakeUpstream::FakeUpstream(uint32_t port, FakeHttpConnection::Type type,
  ...
  ENVOY_LOG(critical, "this [{}] tid [{}] starting fake server on port {}. Address version is {}",static_cast<void *>(this), std::this_thread::get_id(),
...
FakeUpstream::FakeUpstream(Network::TransportSocketFactoryPtr&& transport_socket_factory,
  ...
  ENVOY_LOG(critical, "this [{}] tid[{}] starting fake SSL server on port {}. Address version is {}",static_cast<void *>(this), std::this_thread::get_id(),
            this->localAddress()->ip()->port(), Network::Test::addressVersionAsString(version));
}
...
FakeUpstream::FakeUpstream(Network::TransportSocketFactoryPtr&& transport_socket_factory,
  ...
  ENVOY_LOG(critical, "this [{}] tid [{}] FakeUpstream::FakeUpstream(): spwan thread to poll newly created host dispatcher ",static_cast<void *>(this), std::this_thread::get_id());
...
// event notification entry point?
bool FakeUpstream::createNetworkFilterChain(Network::Connection& connection,
                                            const std::vector<Network::FilterFactoryCb>&) {
  ENVOY_LOG(critical, "this [{}] tid [{}] FakeUpstream::createNetworkFilterChain(): enqueue on new_connection_ and notify new_connection_event_ ",static_cast<void *>(this), std::this_thread::get_id());
  Thread::LockGuard lock(lock_);
...
void FakeUpstream::threadRoutine() {
  ENVOY_LOG(critical, "this [{}] tid [{}] FakeUpstream::threadRoutine(): poll blocking on dispatcher_ ",static_cast<void *>(this), std::this_thread::get_id());
...
AssertionResult FakeUpstream::waitForHttpConnection(Event::Dispatcher& client_dispatcher,
  ...
  ENVOY_LOG(critical,
    "this [{}] tid [{}] FakeUpstream::waitForHttpConnection(): acquire lock, non-blocking poll dispatcher sent as arg while(new_connections_.empty()) non blocking ",static_cast<void *>(this), std::this_thread::get_id());
  ...
  ENVOY_LOG(critical,
    "this [{}] tid [{}] FakeUpstream::waitForHttpConnection(): return FakeHttpConnection to caller ",static_cast<void *>(this), std::this_thread::get_id());
  return AssertionSuccess();
}
...
AssertionResult FakeUpstream::waitForRawConnection(FakeRawConnectionPtr& connection,
  ...
  Thread::LockGuard lock(lock_);
  if (new_connections_.empty()) {
    ENVOY_LOG(critical, "this [{}] tid[{}] waiting for raw connection",static_cast<void *>(this), std::this_thread::get_id());
...
AssertionResult FakeRawConnection::waitForData(uint64_t num_bytes, std::string* data,
                                               milliseconds timeout) {
  Thread::LockGuard lock(lock_);
  ENVOY_LOG(critical, "this [{}] tid [{}] waiting for {} bytes of data",static_cast<void *>(this), std::this_thread::get_id(), num_bytes);
  auto end_time = time_system_.monotonicTime() + timeout;
...
Network::FilterStatus FakeRawConnection::ReadFilter::onData(Buffer::Instance& data,
  ...
  ENVOY_LOG(critical, "this [{}] tid[{}] got {} bytes, end_stream {}",
          static_cast<void *>(this), std::this_thread::get_id(), data.length(), end_stream);
