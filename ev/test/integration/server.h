...
public:
  TestOptionsImpl(const std::string& config_path, Network::Address::IpVersion ip_version)
      : config_path_(config_path), local_address_ip_version_(ip_version),
        ...
        {
            host_only_ = 1; // only collect host events for testing
            // Use trace log level for all testing
  ...
  // Defined in ev/include/envoy/server/options.h
  spdlog::level::level_enum logLevel() const override { log_level_; }
  ...
  std::string ffConfigPath() override {return "path";}
  std::string ffProcType() override {return "type";} 
  uint32_t ffProcId() override {return 1;}
  uint8_t hostOnly() override { return host_only_;}
