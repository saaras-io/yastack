...
void BufferingStreamDecoder::decodeTrailers(Http::HeaderMapPtr&&) { NOT_IMPLEMENTED_GCOVR_EXCL_LINE; }
...
RawConnectionDriver::RawConnectionDriver(uint32_t port, Buffer::Instance& initial_data,
  ...
  dispatcher_ = api_->allocateDispatcherHostOnly(test_time_.timeSystem());
...
void RawConnectionDriver::run(Envoy::Event::Dispatcher::RunType rt) { dispatcher_->run(rt); }
...
IntegrationUtil::makeSingleRequest(const Network::Address::InstanceConstSharedPtr& addr,
  ...
  Event::DispatcherPtr dispatcher(api.allocateDispatcherHostOnly(test_time_.timeSystem()));
