...
IntegrationStreamDecoder::IntegrationStreamDecoder(Event::Dispatcher& dispatcher)
  ...
  ENVOY_LOG_MISC(critical,
          " tid[{}] IntegrationStreamDecoder::IntegrationStreamDecoder() Creating IntegrationStreamDecoder with dispatcher sent as an arg",std::this_thread::get_id());
    }

void IntegrationStreamDecoder::waitForContinueHeaders() {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::waitForContinueHeaders() - blocking poll on dispatcher until continue_headers_ is set ",std::this_thread::get_id());
...
void IntegrationStreamDecoder::waitForHeaders() {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::waitForHeaders() - blocking poll on dispatcher until headers_ is set ",std::this_thread::get_id());
  if (!headers_.get()) {
...
void IntegrationStreamDecoder::waitForBodyData(uint64_t size) {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::waitForBodyData() - blocking poll on dispatcher ",std::this_thread::get_id());
...
void IntegrationStreamDecoder::waitForEndStream() {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::waitForEndStream() - blocking poll on dispatcher until saw_end_stream_",std::this_thread::get_id());
  if (!saw_end_stream_) {
...
void IntegrationStreamDecoder::waitForReset() {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::waitForReset() - blocking poll on dispatcher until saw_reset_",std::this_thread::get_id());
  if (!saw_reset_) {
...
void IntegrationStreamDecoder::decode100ContinueHeaders(Http::HeaderMapPtr&& headers) {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::decode100ContinueHeaders() - exit dispatcher if waiting_for_continue_headers_ set",std::this_thread::get_id());
  continue_headers_ = std::move(headers);
  if (waiting_for_continue_headers_) {
    ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::decode100ContinueHeaders() - exiting dispatcher",std::this_thread::get_id());
    dispatcher_.exit();
  }
}

void IntegrationStreamDecoder::decodeHeaders(Http::HeaderMapPtr&& headers, bool end_stream) {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::decodeHeaders() - exit dispatcher if (end_stream && waiting_for_continue_headers_) set",std::this_thread::get_id());
  saw_end_stream_ = end_stream;
  headers_ = std::move(headers);
  if ((end_stream && waiting_for_end_stream_) || waiting_for_headers_) {
    ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::decodeHeaders() - exiting dispatcher",std::this_thread::get_id());
    dispatcher_.exit();
  }
}
...
void IntegrationStreamDecoder::decodeData(Buffer::Instance& data, bool end_stream) {
  ...
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::decodeData() - exit dispatcher if (end_stream && waiting_for_continue_headers_) set",std::this_thread::get_id());
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::decodeData() - exit dispatcher if (body_data_waiting_length_ == 0) ",std::this_thread::get_id());
  saw_end_stream_ = end_stream;
  ...
  if (end_stream && waiting_for_end_stream_) {
    ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::decodeData() - exiting dispatcher",std::this_thread::get_id());
    dispatcher_.exit();
...
void IntegrationStreamDecoder::decodeTrailers(Http::HeaderMapPtr&& trailers) {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::decodeTrailers() - exit dispatcher if waiting_for_end_stream_ set ",std::this_thread::get_id());
  saw_end_stream_ = true;
  trailers_ = std::move(trailers);
  if (waiting_for_end_stream_) {
    ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::onResetStream() - exiting dispatcher",std::this_thread::get_id());
    dispatcher_.exit();
  }
}

void IntegrationStreamDecoder::onResetStream(Http::StreamResetReason reason) {
  ENVOY_LOG_MISC(critical, " tid[{}] IntegrationStreamDecoder::onResetStream() - exit dispatcher if waiting_for_reset_ is set",std::this_thread::get_id());
  saw_reset_ = true;
...
BaseIntegrationTest::BaseIntegrationTest(Network::Address::IpVersion version,
    ...
    : api_(new Api::Impl(std::chrono::milliseconds(10000))),
      ...
      default_log_level_(spdlog::level::trace) {
...
Network::ClientConnectionPtr BaseIntegrationTest::makeClientConnection(uint32_t port) {
  ENVOY_LOG_MISC(critical,
   " tid[{}] BaseIntegrationTest::makeClientConnection() Creating client connection to localhost:{} using BaseIntegrationTest::dispatcher_",std::this_thread::get_id(), port);
...
void BaseIntegrationTest::createUpstreams() {
  ...
  ENVOY_LOG_MISC(critical, " tid[{}] Creating upstreams",std::this_thread::get_id());
  for (uint32_t i = 0; i < fake_upstreams_count_; ++i) {
    if (autonomous_upstream_) {
      ...
      ENVOY_LOG_MISC(critical, " tid[{}] Enqueued fake upstream in BaseIntegrationTests::fake_upstreams_",std::this_thread::get_id());
    }
  }
}
...
void BaseIntegrationTest::createEnvoy() {
  ...
  ENVOY_LOG_MISC(critical, " tid[{}] Creating envoy using BaseIntegrationTest::createEnvoy()",std::this_thread::get_id());
  for (auto& upstream : fake_upstreams_) {
    if (upstream->localAddress()->ip()) {
      ENVOY_LOG_MISC(critical, " tid[{}] Saving port {} in ports",std::this_thread::get_id(), upstream->localAddress()->ip()->port());
      ports.push_back(upstream->localAddress()->ip()->port());
    }
  }
  config_helper_.finalize(ports);

  ENVOY_LOG_MISC(critical, " tid[{}] Running Envoy with configuration {}",std::this_thread::get_id(),
