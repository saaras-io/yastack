...
public:
  ...
  std::string ffConfigPath() override { return ff_config_path_;}
  std::string ffProcType() override { return ff_proc_type_; }
  uint32_t ffProcId() override { return ff_proc_id_; }
...
#if defined (__HAS_FF__)
  // ff args
  std::string ff_config_path_;
  std::string ff_proc_type_;
  uint32_t    ff_proc_id_;
  // Takes values (0, 1) to indicate (host_and_fp events, host only events) respectively
  uint8_t     host_only_;
#endif
};
