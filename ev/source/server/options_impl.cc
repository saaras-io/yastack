...
OptionsImpl::OptionsImpl(int argc, const char* const* argv,
  ...
  // By default, we create a dispatcher that polls both the host and fp stack
  host_only_ = 0;
  ...
  // All commands are accepted through OptionsImpl interface (including ff args)
  TCLAP::ValueArg<std::string> ff_config_path("f", "conf-file", "Path to fs configuration file", false,
                                           "", "string", cmd);

  TCLAP::ValueArg<std::string> ff_proc_type("t", "proc-type", "proc type [primary] or [secondary]", false,
                                           "", "string", cmd);

  TCLAP::ValueArg<uint32_t> ff_proc_id("p", "proc-id", "proc-id [0] or [1] or .. [n]", false, 0,
                                          "uint32_t", cmd);
#endif
...
#if 0
  // TODO
  // For now for testing, just set the default log level to trace
  log_level_ = static_cast<spdlog::level::level_enum>(0);
#endif
...
#if defined (__HAS_FF__)
  // ff args
  ff_config_path_ = ff_config_path.getValue();
  ff_proc_type_   = ff_proc_type.getValue();
  ff_proc_id_     = ff_proc_id.getValue();
#endif
