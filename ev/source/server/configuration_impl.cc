...
InitialImpl::InitialImpl(const envoy::config::bootstrap::v2::Bootstrap& bootstrap) {
  ...
  if (admin.address().socket_address().provider() == envoy::api::v2::core::SocketAddress::HOST) {
    admin_.address_->provider(Envoy::Network::Address::SocketProvider::Host);
  } else if (admin.address().socket_address().provider() == envoy::api::v2::core::SocketAddress::FP) {
    admin_.address_->provider(Envoy::Network::Address::SocketProvider::Fp);
  }
