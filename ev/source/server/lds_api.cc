...
static void setListenerSocketProvider(const ::envoy::api::v2::Listener &listener,
        ::envoy::api::v2::core::SocketAddress_SocketProvider provider) {
    const ::envoy::api::v2::Listener * lptr = &listener;
    ::envoy::api::v2::core::Address* address = (const_cast<::envoy::api::v2::Listener *>(lptr))->mutable_address();
    ::envoy::api::v2::core::SocketAddress* mutable_sock_addr = address->mutable_socket_address();
    mutable_sock_addr->set_provider(provider);
}
...
void LdsApiImpl::onConfigUpdate(const ResourceVector& resources, const std::string& version_info) {
  ...
  for (const auto& listener : resources) {
    const std::string listener_name = listener.name();
        ...
        // Listener updates from management server create only isteners on the FP stack.
        // Overwrite the socket type to FP. Control plane isn'aware that it is programming
        // a socket of type FP. This makes the control agnostic of the underlying data path.
      setListenerSocketProvider(listener,
              envoy::api::v2::core::SocketAddress_SocketProvider_FP);
#endif
    try {
