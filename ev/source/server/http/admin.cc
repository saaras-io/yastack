...
extern "C" void ff_enableFpLogging();
extern "C" void ff_disableFpLogging();
...
bool AdminImpl::changeLogLevel(const Http::Utility::QueryParams& params) {
  ...
  if (name == "level") {
    ...
    /* Enable/disable fast path logging only when we are tracing */
    if (logger_to_change->name() == "fp") {
      if (level_to_use == spdlog::level::trace) {
        ff_enableFpLogging();
      } else {
        ff_disableFpLogging();
      }
