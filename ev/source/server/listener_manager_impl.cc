...
ProdListenerComponentFactory::createListenSocket(Network::Address::InstanceConstSharedPtr address,
                                                 ...
                                                 bool bind_to_port, uint8_t provider) {
  ...
  // Unix domain copy, not supported for FF
  const int fd = server_.hotRestart().duplicateParentListenSocket(addr);
  if (fd != -1) {
    ENVOY_LOG(debug, "obtained socket for address {} from parent", addr);
    return std::make_shared<Network::TcpListenSocket>(fd, address, options);
  } else {
    return std::make_shared<Network::TcpListenSocket>(address, bind_to_port, options, provider);
  }
}
...
ListenerImpl::ListenerImpl(const envoy::api::v2::Listener& config, const std::string& version_info,
    ...
    : parent_(parent), address_(Network::Address::resolveProtoAddress(config.address())),
      ...
      provider_(config.address().socket_address().provider()),
  ...
  ENVOY_LOG(debug, "created listener name={} provider={}", name_, provider_);
...
ListenerManagerImpl::ListenerManagerImpl(Instance& server,
    ...
    : server_(server), time_source_(time_source), 
    factory_(listener_factory), stats_(generateStats(server.stats())) {
    workers_.emplace_back(worker_factory.createWorker(main_thread_dispatcher));
}
#endif
...
bool ListenerManagerImpl::addOrUpdateListener(const envoy::api::v2::Listener& config,
  ...
  ENVOY_LOG(debug, "begin add/update listener: name={} hash={} provider={}", name, hash, config.address().socket_address().provider());
...
void ListenerManagerImpl::startWorkers(GuardDog& guard_dog) {
  ...
  for (const auto& worker : workers_) {
    ...
    for (const auto& listener : active_listeners_) {
      ENVOY_LOG(info, "Addition listener name={} provider={} ", listener->name(), listener->provider());
      addListenerToWorker(*worker, *listener);
    }
