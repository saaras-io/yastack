...
MainCommonBase::MainCommonBase(OptionsImpl& options) : options_(options) {
 ...
 ff_argv[n++] = strdup(options_.ffConfigPath().c_str());
  
 // --proc-type=primary OR --proc-type=secondary
 std::string proc_type("--proc-type=" + options_.ffProcType());
 ff_argv[n++] = strdup(proc_type.c_str());
  
 // --proc-id=0 or --proc-id=1 etc.
 std::string proc_id("--proc-id=" + std::to_string(options_.ffProcId()));
 printf("PROC ID received [%u] [%s]\n", options_.ffProcId(), proc_id.c_str());
 ff_argv[n++] = strdup(proc_id.c_str());

 ff_argc = n;
 ff_init(ff_argc, ff_argv);
 ff_regist_packet_dispatcher(toeplitz_dispatch);
#endif
