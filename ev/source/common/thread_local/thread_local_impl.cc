...
void InstanceImpl::runOnAllThreads(Event::PostCb cb) {
  ...
  ENVOY_LOG_MISC(debug, "tid [{}] InstanceImpl::runOnAllThreads () - invoking on dispatchers() ",std::this_thread::get_id());
  for (Event::Dispatcher& dispatcher : registered_threads_) {
    ENVOY_LOG_MISC(debug, "tid [{}] InstanceImpl::runOnAllThreads () - posting  on dispatcher ",std::this_thread::get_id());
    dispatcher.post(cb);
  }
  ENVOY_LOG_MISC(debug, "tid [{}] InstanceImpl::runOnAllThreads () - invoking locally ",std::this_thread::get_id());
...
void InstanceImpl::runOnAllThreads(Event::PostCb cb, Event::PostCb all_threads_complete_cb) {
  ...
  //ENVOY_LOG_MISC(debug, "tid [{}] InstanceImpl::runOnAllThreads (cb, all_threads_done_cb) invoking callback on main thread ",std::this_thread::get_id());
  cb();
  std::shared_ptr<std::atomic<uint64_t>> worker_count =
      std::make_shared<std::atomic<uint64_t>>(registered_threads_.size());
  //ENVOY_LOG_MISC(debug, "tid [{}] InstanceImpl::runOnAllThreads (cb, all_threads_done_cb) worker_count [{}]",std::this_thread::get_id(), registered_threads_.size());
  for (Event::Dispatcher& dispatcher : registered_threads_) {
    //ENVOY_LOG_MISC(debug, "tid [{}] InstanceImpl::runOnAllThreads (cb, all_threads_done_cb) posting on registered thread - ",std::this_thread::get_id());
    dispatcher.post([this, worker_count, cb, all_threads_complete_cb]() -> void {
      //ENVOY_LOG_MISC(debug, "tid [{}] InstanceImpl::runOnAllThreads (cb, all_threads_done_cb) on posted thread - worker_count [{}] ",std::this_thread::get_id(), worker_count);
      cb();
      if (--*worker_count == 0) {
        //ENVOY_LOG_MISC(debug, "tid [{}] InstanceImpl::runOnAllThreads (cb, all_threads_done_cb) worker_count zero, post all_threads_complete_cb on main thread ",std::this_thread::get_id());
  ...
  // TODO: Put comment here why we need this
  // TODO: Document the new threading model
