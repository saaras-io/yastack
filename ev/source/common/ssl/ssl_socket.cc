...
#if __HAS_FF__
  int64_t use_fd = callbacks_->fd(); 
  BIO* bio;

  if (IS_FP_SOCKET(use_fd)) {
      CLEAR_FP_SOCKET(use_fd);
      bio = BIO_new_socket((int) use_fd, 0);
      // Tell BIO layer that it needs to read from FS and not host stack
      bio->fs_socket = 1;
  } else {
      bio = BIO_new_socket((int) use_fd, 0);
      // BIO_new() in boringssl/crypto/bio/bio.c already does this
      bio->fs_socket = 0;
  }
#else
