...
void ThreadLocalStoreImpl::mergeHistograms(PostMergeCb merge_complete_cb) {
  ENVOY_LOG_MISC(debug, "tid [{}] ThreadLocalStoreImpl::mergeHistograms()",std::this_thread::get_id());
  if (!shutting_down_) {
    ASSERT(!merge_in_progress_);
    merge_in_progress_ = true;
    //ENVOY_LOG_MISC(debug, "tid [{}] ThreadLocalStoreImpl::mergeHistograms() invoking runOnAllThreads",std::this_thread::get_id());
    tls_->runOnAllThreads(
        [this]() -> void {
          //ENVOY_LOG_MISC(debug, "tid [{}] ThreadLocalStoreImpl::runOnAllThreads() ",std::this_thread::get_id());
...
void ThreadLocalStoreImpl::mergeInternal(PostMergeCb merge_complete_cb) {
  //ENVOY_LOG_MISC(debug, "tid [{}] ThreadLocalStoreImpl::mergeInternal() ",std::this_thread::get_id());
  if (!shutting_down_) {
    for (const ParentHistogramSharedPtr& histogram : histograms()) {
      histogram->merge();
    }

    //ENVOY_LOG_MISC(debug, "tid [{}] ThreadLocalStoreImpl::mergeInternal() - invoking merge_complete_cb() ",std::this_thread::get_id());
    merge_complete_cb();
