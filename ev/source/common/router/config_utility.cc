...
#define MAX_BODY_SIZE_FOR_DIRECT_RESPONSE	(1 << 31)
...
std::string ConfigUtility::parseDirectResponseBody(const envoy::api::v2::route::Route& route) {
  static const uint64_t MaxBodySize = MAX_BODY_SIZE_FOR_DIRECT_RESPONSE;
  ...
  if (!filename.empty()) {
    ...
    uint64_t size = Filesystem::fileSize(filename);
    if (size < 0) {
