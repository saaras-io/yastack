...
#if defined (__HAS_FF__)
      // In this loop, the read call above does a sleep which relinquishes the CPU.
      // since in non-blocking dpdk, there is no sleep implementation,
      // Break out of the loop right now
      // We resume implementation once there is event on the descriptor.
      // The resumption will happen when the event loop processes that event.
      break;
#endif
    }
  } while (true);
