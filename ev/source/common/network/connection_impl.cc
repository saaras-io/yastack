...
void ConnectionImpl::noDelay(bool enable) {
  ...
  if (likely(IS_FP_SOCKET(fd()))) {
      ...
      int rc = ff_getsockname(use_fd, &addr, &len);

      RELEASE_ASSERT(rc == 0, "");
      if (addr.sa_family == AF_UNIX) {
          return;
      }
      rc = ff_setsockopt(use_fd, IPPROTO_TCP, TCP_NODELAY, &new_value, sizeof(new_value));
  } else {
      sockaddr addr;
      socklen_t len = sizeof(addr);
      int rc = getsockname(use_fd, &addr, &len);

      RELEASE_ASSERT(rc == 0, "");
      if (addr.sa_family == AF_UNIX) {
          return;
      }
      rc = setsockopt(use_fd, IPPROTO_TCP, TCP_NODELAY, &new_value, sizeof(new_value));
  }
...
void ConnectionImpl::onWriteReady() {
  ...
  if (connecting_) {
    ...
    int rc;
    if (likely(IS_FP_SOCKET(fd()))) {
        rc = ff_getsockopt((fd() & 0xFFFFFFFF), SOL_SOCKET, SO_ERROR, &error, &error_size);
    } else {
        rc = getsockopt(fd(), SOL_SOCKET, SO_ERROR, &error, &error_size);
    }
    ASSERT(0 == rc);
