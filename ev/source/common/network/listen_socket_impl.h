...
protected:
  SocketImpl(int64_t fd, const Address::InstanceConstSharedPtr& local_address)
      : fd_(fd), local_address_(local_address) {}

  /* We use 64-bits here (instead of using an int).
   * This is to differenciate a regular socket from a fs based socket.
   */
  int64_t fd_;
...
protected:
  ListenSocketImpl(int64_t fd, const Address::InstanceConstSharedPtr& local_address)
...
public:
  ...
  TcpListenSocket(const Address::InstanceConstSharedPtr& address,
                  bool bind_to_port, const Network::Socket::OptionsSharedPtr& options, uint8_t provider);
  TcpListenSocket(int64_t fd, const Address::InstanceConstSharedPtr& address,
...
public:
  ConnectionSocketImpl(int64_t fd, const Address::InstanceConstSharedPtr& local_address,
...
public:
  AcceptedSocketImpl(int64_t fd, const Address::InstanceConstSharedPtr& local_address,
                     const Address::InstanceConstSharedPtr& remote_address)
...
public:
  ...
  /* TODO: Look at the downstream socket provider and use the same provider */
  ClientSocketImpl(const Address::InstanceConstSharedPtr& remote_address, uint8_t provider)
      : ConnectionSocketImpl(remote_address->socket(provider, Address::SocketType::Stream), nullptr,
                             remote_address) {}
