...
public:
  ...
  SocketProvider provider() const override;
  void provider(const SocketProvider provider) override;

protected:
  InstanceBase(Type type) : type_(type) {}
  int64_t socketFromSocketType(SocketType type) const;
  int64_t socketFromSocketType(uint8_t provider, SocketType type) const;
...
public:
  ...
  int64_t socket(SocketType type) const override;
  int64_t socket(uint8_t provider, SocketType type) const override;
