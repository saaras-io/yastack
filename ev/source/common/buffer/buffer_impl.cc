...
Api::SysCallIntResult OwnedImpl::read(int64_t fd, uint64_t max_length) {
  if (max_length == 0) {
    return {0,0};
  }
  ...
  Api::SysCallSizeResult result;
    if (likely(IS_FP_SOCKET(fd))) {
        CLEAR_FP_SOCKET(fd);
        result.rc_ = ff_readv(fd, iov, static_cast<int>(num_slices_to_read));
        result.errno_ = errno;
    } else {
        result = os_syscalls.readv(fd, iov, static_cast<int>(num_slices_to_read));
    }
  if (result.rc_ < 0) {
...
//int OwnedImpl::read(int64_t fd, uint64_t max_length) {
//    if (likely(IS_FP_SOCKET(fd))) {
//        CLEAR_FP_SOCKET(fd);
//        return evbuffer_read(buffer_.get(), (int) fd, max_length);
//    } else {
//        return evbuffer_read_host(buffer_.get(), (int) fd, max_length);
//    }
//}
//
//
...
Api::SysCallIntResult OwnedImpl::write(int64_t fd) {
  ...
  Api::SysCallSizeResult result;
    if (likely(IS_FP_SOCKET(fd))) {
        CLEAR_FP_SOCKET(fd);
        result.rc_ = ff_writev(fd, iov, num_slices_to_write);
        result.errno_ = errno;
    } else {
        result = os_syscalls.writev(fd, iov, num_slices_to_write);
    }
  if (result.rc_ > 0) {
...
//int OwnedImpl::write(int64_t fd) {
//
//    if (likely(IS_FP_SOCKET(fd))) {
//        CLEAR_FP_SOCKET(fd);
//        return evbuffer_write(buffer_.get(), (int) fd);
//    } else {
//        return evbuffer_write_host(buffer_.get(), (int) fd);
//    }
//}
