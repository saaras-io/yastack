...
// This is just a wrapper on OwnedImpl with the additional calls to
// checkHighWatermark() during add-to-buffer operations AND
// checkLowWatermark() during remove-from-buffer operations
//
// OwnedImpl in turn is a wrapper over evbuffer_ which in turn holds an array of iovec
...
Api::SysCallIntResult WatermarkBuffer::read(int64_t fd, uint64_t max_length) {
...
Api::SysCallIntResult WatermarkBuffer::write(int64_t fd) {
